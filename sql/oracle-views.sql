...
-- =============================================================================
-- 目标: Oracle 12c+
-- 用途: 将现有业务表映射为前端所需的 12 个视图
-- 使用: 
--   1. 将 FROM 子句中的表名替换为实际的业务表名。
--   2. 如果源表列名与下面引号内名称不一致，请同步修改 SELECT 中的列引用。
--   3. 注意：所有列名统一使用双引号小写，避免 YEAR/MONTH/TIME 等标识符冲突。
...
--       fixed_code, unit, brand, supplier, equip_value, asset_value,
--       alloc_value, maker, operate_date, serial_no, source, office, user_name,
--       handover_time, virtual_flag, parent_code, remark, detail, approve_time
...
-- GRANT SELECT ON repairs_single_device TO 用户名;
-- GRANT SELECT ON repairs_department TO 用户名;
-- GRANT SELECT ON monthly_fuel TO 用户名;
-- GRANT SELECT ON department_fuel TO 用户名;
-- GRANT SELECT ON department_energy TO 用户名;
